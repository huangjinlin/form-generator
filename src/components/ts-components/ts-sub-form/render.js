...
tsKeys.forEach(key => {
  ...
  tsComponentChild[tsTag] = tsSlotsFiles(key).default
})
...
function tsMountSlotFiles(h, tsRawData, tsChildren) {
  const childObjs = tsComponentChild[tsRawData.__config__.tag]
  if (childObjs) {
...
function tsClearAttrs(tsDataObject) {
  delete tsDataObject.attrs.__config__
...
export default {
  ...
  render(h) {
    ...
    return h(this.data.__config__.tag, tsDataObject, tsChildren)
  }
}
